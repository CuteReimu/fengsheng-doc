...
::: warning 注意
在线下游戏时，当白小年使用[密令](../welcome/welcome.md#卡牌效果)时，被[密令](../welcome/welcome.md#卡牌效果)的玩家选择了传出情报的目标时，很显然所有的玩家此时已经知道他声明的传出情报的目标和锁定的目标；线上由于技术原因，此时大家看不到具体的目标，等白小年发动**转交**后才能看到（但确实目标已经被选好确定下来了）。此区别影响不大（对白小年选择**转交**目标的决策存在一定的影响），暂时不会调整。
:::
